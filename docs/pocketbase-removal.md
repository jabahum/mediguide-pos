...
- The Flutter health-infrastructure list and its region, district, facility
  ...
  Facility and geography writes now use the dedicated `facility.write`
  permission instead of borrowing `guideline.write`; supported read-only roles
  receive `facility.read` without write access.
