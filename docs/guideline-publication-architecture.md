...
This does not complete the overall migration. The new tables are not yet populated by ingestion or exposed by an API. The next safe slice is the version capability manifest followed by draft-only structured ingestion. It must not modify the current progress foreign key or delete `medical_guidelines`.
