...
This does not complete the overall migration. Structured tables are not yet populated by ingestion or exposed by a public API. The next safe slice is Phase 4 draft-only structured ingestion. It must not modify the current progress foreign key or delete `medical_guidelines`.
