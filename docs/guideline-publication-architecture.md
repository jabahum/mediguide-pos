...
This does not complete the overall migration. Structured extraction requires editorial correction and approval before publication and is not yet exposed by a public API. The next safe slice is Phase 5 editorial review and publication validation. It must not modify the current progress foreign key or delete `medical_guidelines`.
