...
- Support tickets and replies are migrated to typed nested routes. Ticket
  ...
  `SupportRepository`. The former ticket/reply specifications and routing-map
  entries are removed. FAQ, FAQ-tag, and documentation migration remains.
...
| Support — tickets/replies migrated; help content remaining | Former compatibility resources: `support_tickets`, `support_ticket_replies`; remaining: `faqs`, `faq_tags`, `documentation` | Dashboard support tickets use `SupportTicketsService`; mobile help center uses `SupportRepository`. FAQ and documentation screens remain transitional. | JWT-owned ticket/reply create, staff assignment/status/internal notes; editor FAQ/docs CRUD remains. | Typed `/api/v2/support/tickets`, `/api/v2/support/tickets/{id}`, and `/api/v2/support/tickets/{id}/replies`; transitional `/api/v2/faqs`, `/api/v2/faq-tags`, and `/api/v2/documentation`. | User owns tickets; support staff triage/reply; editor/admin publish help content. |
...
Calculators, decision tools, drugs, users/roles/permissions, and facilities
with geographic reference data are complete. The next required slice is
support and help content, followed by consultants. The remaining guideline
